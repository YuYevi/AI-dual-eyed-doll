...
static bool DecompressEyeAsset(const EyeAsset& asset, uint16_t* dest) {
    // tinfl_decompress_mem_to_mem() keeps tinfl_decompressor on the caller's stack.
    // Board init runs on the default 8KB main task, so allocate the decompressor.
    ...
    // Packed RGB565 assets are little-endian. GC9D01 4-SPI wants the high byte first.
